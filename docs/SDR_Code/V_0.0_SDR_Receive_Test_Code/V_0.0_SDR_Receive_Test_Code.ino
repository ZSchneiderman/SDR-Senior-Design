//Receive Test code for SDR, from Charlie Morris, call sign: ZL2CT, additional annotations added by James Bell
// Libraries
...
// Iowa Hills Hilbert transform filter coefficients (Why are these needed?)
...
AudioConnection         patchCord25(RX_Summer, 0, audioOutput, 0);            // Receiver summer to receiver LPF (Low Power Frequency??)
...
void Turn_On_Receiver()
{
  ...
  audioShield.lineInLevel(5);                                               // Default is 5 (why?)
  ...
  if (freq <= 9999999)          // LSB (lower Sideband)
  {
    RX_Summer.gain(0, 1);
    RX_Summer.gain(1, -1);
  }
  if (freq > 9999999)           // USB (Upper Sideband)
  {
    RX_Summer.gain(0, 1);
    RX_Summer.gain(1, 1);
  }
