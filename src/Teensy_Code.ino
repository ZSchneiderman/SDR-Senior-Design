...
//bandpassRF, closet to antenna 
//pin = 32
//low = 80 meter band
//high = 20 meter band 

//bandpassSplitter, next to splitter and after bandpass filter
//pin = 31
//low = 80 meter band
//high = 20 meter band

//RelayTR, between antena and RF AMP
//pin = 30
//low = recieve
//high = transmit
...
const long bandInit =  3750000;    // the bandInit is st to the USA 80 Meter AM calling frequency
...
volatile int bandPick = 1;     // this picks the 20 or 80 meter bands , 0:20, 1:80
...
static const int relayBandSelect = 30;
static const int relaySplitter = 31;
static const int relayTXRX = 32;
static const int ptt_bt = 29;
static const int led = 13;
static const int ssb_bt = 28;
static const int license_bt = 27;
...
volatile int license = 0;                 // this denotes the licence level, 0 is unlicenced
volatile bool ssb = false;                // false = LSB     true = USB
...
AudioFilterFIR          TX_Hilbert_Plus_45;                                   // Name for the RX +45 Hilbert transform
AudioFilterFIR          TX_Hilbert_Minus_45;                                  // Name for the RX -45 Hilbert transform
AudioMixer4             TX_I_Sideband_Switch;                                 // Name for the sideband switching summer for the I channel, 
                                                                              // The I sideband switch tells the system to invert or not invert the in phase signal

// Audio connections
//the sideband selection changes only the +45 degree side of the signal
//by inverting it as needed. 
...
void setup()
{
  ...
  pinMode(relayBandSelect, OUTPUT);
  pinMode(relaySplitter, OUTPUT);
  pinMode(relayTXRX, OUTPUT);
  pinMode(led, OUTPUT);
  pinMode(ssb_bt, INPUT_PULLDOWN);
  pinMode(license_bt, INPUT_PULLDOWN);

  
  digitalWrite(relayBandSelect, HIGH);
  digitalWrite(relaySplitter, LOW);
  digitalWrite(relayTXRX, LOW);
  ...
  attachInterrupt(digitalPinToInterrupt(ptt_bt), pttISR, CHANGE);
  attachInterrupt(digitalPinToInterrupt(ssb_bt), ssbISR, RISING);
  attachInterrupt(digitalPinToInterrupt(license_bt), licenseISR, RISING);

    // This is Initalizing the clock generator 
    si5351.init(SI5351_CRYSTAL_LOAD_8PF, 0, 0);
    si5351.set_pll(SI5351_PLL_FIXED, SI5351_PLLA);
    si5351.drive_strength(SI5351_CLK0, SI5351_DRIVE_8MA);
    si5351.set_freq((freq * 100ULL)-14700, SI5351_CLK0);  
    si5351.drive_strength(SI5351_CLK2, SI5351_DRIVE_8MA);
    si5351.set_freq((freq * 100ULL)-14700, SI5351_CLK2); 
  ...
  updateDisplay();
  digitalWrite(relayBandSelect, LOW);
}
...
void loop()
{
  ...
  if(freq < eMbStart){
    ...
    bandPick = 0;
    digitalWrite(relayBandSelect, HIGH);
    digitalWrite(relaySplitter, HIGH);
...
//this function is called when the ppt button is pressed
//causing the system to switch to transmit mode
void pttISR()
{
 
  if((license == 3 && ((freq > 3800000 && freq < 4000000) || (freq > 14225000 && freq < 14350000)) || license == 4)){
    AudioNoInterrupts();
    digitalWrite(relayTXRX, HIGH);
    ...
    if (ssb)           // USB
    {
      // invert the signal with the -1 for upper sideband
      TX_I_Sideband_Switch.gain(0, -1);
    }
...
void ssbISR()
{
  detachInterrupt(digitalPinToInterrupt(ssb_bt));
  displayFlag = 1;       //this tells the machine in the next loop to update the LCD
  ssb = !ssb;
...
void licenseISR()
{
  detachInterrupt(digitalPinToInterrupt(license_bt));
  displayFlag = 1;       //this tells the machine in the next loop to update the LCD
  license = license + 1;
  if(license >= 4){
    license = 0;
...
void SendFrequency()
{
  EvenDivisor();
  if(ssb){
  si5351.set_freq_manual((14700+freq) * SI5351_FREQ_MULT, Even_Divisor * (14700+freq) * SI5351_FREQ_MULT, SI5351_CLK0);
  si5351.set_freq_manual((14700+freq) * SI5351_FREQ_MULT, Even_Divisor * (14700+freq) * SI5351_FREQ_MULT, SI5351_CLK2);
  }
  else{
    si5351.set_freq_manual((freq-14700) * SI5351_FREQ_MULT, Even_Divisor * (freq-14700) * SI5351_FREQ_MULT, SI5351_CLK0);
    si5351.set_freq_manual((freq-14700) * SI5351_FREQ_MULT, Even_Divisor * (freq-14700) * SI5351_FREQ_MULT, SI5351_CLK2);
  }
